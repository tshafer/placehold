<x-layout>
    <div class="container mx-auto px-4 sm:px-6 lg:px-8 max-w-2xl text-center py-24">
        <div class="mb-8">
            <span class="text-8xl font-bold text-gray-200 dark:text-gray-800">500</span>
        </div>
        <h1 class="text-3xl font-bold text-gray-900 dark:text-white mb-4">Something went wrong</h1>
        <p class="text-gray-600 dark:text-gray-400 mb-8">
            We're having trouble processing your request. Please try again in a moment.
        </p>
        <a href="/"
           class="inline-flex items-center px-6 py-3 bg-primary-600 hover:bg-primary-700 text-white font-medium rounded-lg transition-colors">
            Go Home
        </a>
    </div>
</x-layout>
