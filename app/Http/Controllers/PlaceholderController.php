...
class PlaceholderController extends Controller
{
    public function __invoke(
        ...
        Log::info('PlaceholderController invoked', [
            'size' => $size,
        ...
        if (count($dimensions) === 1) {
            ...
            Log::warning('Invalid size parameter', ['size' => $size]);
        ...
        $cachedImage = Cache::remember($cacheKey, now()->addWeek(), function () use ($format, $width, $height, $text, $backgroundColor, $textColorArray, $borderColor, $quality, $font, $textSize, $watermark, $watermarkSize, $watermarkOpacity, $blur, $grayscale, $invert) {
            if ($format === 'svg') {
                Log::info('Generating SVG image');
                ...
                Log::info('Generating raster image');
            ...
            Log::info('Image generated and cached', ['format' => $format, 'contentType' => $contentType]);
    ...
    private function generateRasterImage($width, $height, $text, $backgroundColor, $textColorArray, $borderColor, $format, $quality, $font, $textSize, $watermark, $watermarkSize, $watermarkOpacity, $blur, $grayscale, $invert)
    {
        ...
        if (!file_exists($fontPath)) {
            Log::error('Font file not found', ['font' => $font]);
        ...
        // Add watermark
        if (!empty($watermark)) {
            Log::info('Adding watermark to image');
